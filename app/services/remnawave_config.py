...
REMNAWAVE_URL_KEY = "remnawave_api_url"


async def get_remnawave_api_url(session) -> str | None:
    value = await get_setting(session, REMNAWAVE_URL_KEY, None)
    return str(value) if value else (settings.remnawave_api_url or None)


async def set_remnawave_api_url(session, url: str) -> None:
    await set_setting(session, REMNAWAVE_URL_KEY, (url or "").strip())
...
async def load_remnawave_settings() -> None:
    """Применить сохранённые адрес/токен/ноду к клиенту (при старте)."""
    from app.context import get_remnawave

    async with session_scope() as session:
        api_url = await get_remnawave_api_url(session)
